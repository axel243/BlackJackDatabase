        ...
        static void ResultGame(Speler dealer) {
            foreach (Speler speler in spelerLijst) {
                ...
                if (speler.WaardeHand < 22) {
                    if (speler.Naam != "Dealer" && dealer.WaardeHand < speler.WaardeHand) {
                        Console.WriteLine(speler.Naam + " wint van de dealer! " + speler.WaardeHand);
                        uitslagDBO.AddUitslag(speler, "Gewonnen");
                    } else if(speler.Naam != "Dealer" && dealer.WaardeHand > speler.WaardeHand)
                    {
                        uitslagDBO.AddUitslag(speler, "Verloren");
                    }else if (speler.Naam == "Dealer")
                    {
                        Console.WriteLine("De dealer wint!");
                    }
                }
